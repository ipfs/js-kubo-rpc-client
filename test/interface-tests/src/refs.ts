...
export function testRefs (factory: Factory, options: object) {
  ...
  describe('.refs', function () {
    ...
    for (const [name, options] of Object.entries(getRefsTests())) {
      ...
      it(name, async function () {
        ...
        if (expectTimeout != null) {
          return await expect(all(ipfs.refs(p, params))).to.eventually.be.rejected
        ...
        if (expectError != null) {
          return await expect(all(ipfs.refs(p, params))).to.be.eventually.rejected.and.be.an.instanceOf(Error)
        }
    ...
    it('should respect timeout option when listing refs', async () => {
      return await testTimeout(async () => await drain(ipfs.refs('/ipfs/QmPDqvcuA4AkhBLBuh2y49yhUB98rCnxPxa3eVNC1kAbS1/foo/bar/baz.txt', {
        timeout: 1
      })))
    })
    ...
    it('should get refs with cbor links', async function () {
      ...
      // Call out to IPFS
      const refs = await all(ipfs.refs(`/ipfs/${dagRootCid.toString()}`, { recursive: true }))
...
function getRefsTests (): Record<string, { path?: (cid: CID) => string | string[], params: { edges?: boolean, format?: string, recursive?: boolean, unique?: boolean, maxDepth?: number, timeout?: number }, expected: string[], expectError?: boolean, expectTimeout?: boolean }> {
  return {
    ...
    'should follow a path, <hash>/<subdir>': {
      path: (cid) => `/ipfs/${cid.toString()}/animals`,
    ...
    'should follow a path, <hash>/<subdir>/<subdir>': {
      path: (cid) => `/ipfs/${cid.toString()}/animals/land`,
      params: { format: '<linkname>' },
      expected: [
        'african.txt',
        'americas.txt',
    ...
    'should print refs for multiple paths': {
      path: (cid) => [`/ipfs/${cid.toString()}/animals`, `/ipfs/${cid.toString()}/fruits`],
      params: { format: '<linkname>', recursive: true },
      expected: [
        'land',
        'african.txt',
...
async function loadPbContent (ipfs: KuboClient, node: any) {
  const store: Store = {
    putData: async (data) => {
      return await ipfs.block.put(
        dagPB.encode({
          Data: data,
          Links: []
        })
    ...
    putLinks: async (links) => {
      return await ipfs.block.put(dagPB.encode({
  ...
  return await loadContent(ipfs, store, node)
}

async function loadDagContent (ipfs: KuboClient, node: any) {
  const store: Store = {
    putData: async (data) => {
      ...
      return await ipfs.block.put(serialized)
    },
    putLinks: async (links) => {
...
async function loadContent (ipfs: KuboClient, store: Store, node: any): Promise<CID> {
  if (node instanceof Uint8Array) {
    return await store.putData(node)
  }
  ...
  if (typeof node === 'object') {
    ...
    return await store.putLinks(res)
  }
