...
export async function createTwoShards (ipfs: KuboClient, fileCount: number) {
  ...
  await expect(isShardAtPath(`/ipfs/${dirWithAllFiles.toString()}`, ipfs)).to.eventually.be.true()
  await expect(isShardAtPath(`/ipfs/${dirWithSomeFiles.toString()}`, ipfs)).to.eventually.be.true()
