...
const echoUrl = (text: string) => `${process.env.ECHO_SERVER ?? 'undefined'}/download?data=${encodeURIComponent(text)}`
const redirectUrl = (url: string) => `${process.env.ECHO_SERVER ?? 'undefined'}/redirect?to=${encodeURI(url)}`

export function testAdd (factory: Factory, options: object) {
  ...
  describe('.add', function () {
    ...
    it('should receive file name from progress event', async () => {
      ...
      function handler (p: number, name?: string) {
        receivedName = name
      }
    ...
    it('should wrap content in a directory', async () => {
      ...
      const stats = await ipfs.files.stat(`/ipfs/${wrapper.cid.toString()}/testfile.txt`)

      expect(stats.cid.toString()).to.equal(fixtures.smallFile.cid.toString())
    })
    ...
    describe('with sharding', function () {
      ...
      before(async function () {
        ...
        // @ts-expect-error js-ipfsd-ctl uses interface types
        ipfs = ipfsd.api
      })
