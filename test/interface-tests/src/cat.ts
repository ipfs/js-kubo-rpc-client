...
export function testCat (factory: Factory, options: object) {
  ...
  describe('.cat', function () {
    ...
    it('should respect timeout option when catting files', async function () {
      await testTimeout(async () => await drain(ipfs.cat(CID.parse('QmPDqvcuA4AkhBLBuh2y49yhUB98rCnxPxa3eVNC1kAbS1'), {
        timeout: 1
      })))
    })
    ...
    it('should cat with IPFS path', async () => {
      const ipfsPath = `/ipfs/${fixtures.smallFile.cid.toString()}`
    ...
    it('should cat with IPFS path, nested value', async () => {
      ...
      const data = uint8ArrayConcat(await all(ipfs.cat(`/ipfs/${file.cid.toString()}/testfile.txt`)))
    ...
    it('should cat with IPFS path, deeply nested value', async () => {
      ...
      const data = uint8ArrayConcat(await all(ipfs.cat(`/ipfs/${file.cid.toString()}/b/testfile.txt`)))
      expect(uint8ArrayToString(data)).to.contain('Plz add me!')
    })
    ...
    it('should error on unknown path', async () => {
      return await expect(drain(ipfs.cat(`${fixtures.smallFile.cid.toString()}/does-not-exist`))).to.eventually.be.rejected()
