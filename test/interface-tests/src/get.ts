...
const content = (name: keyof typeof fixtures.directory.files, path?: string) => {
  if (path == null) {
    path = name
  }
...
export function testGet (factory: Factory, options: object) {
  ...
  describe('.get', function () {
    ...
    async function * gzipped (source: AsyncIterable<Uint8Array> | Iterable<Uint8Array>) {
    ...
    async function * tarballed (source: AsyncIterable<Uint8Array> | Iterable<Uint8Array>) {
      yield * pipe(
        source,
        extract(),
    ...
    async function collect (source: AsyncIterable<any> | Iterable<any>) {
      return await all(source)
    }
    ...
    describe('files', function () {
      it('should respect timeout option when getting files', async function () {
        await testTimeout(async () => await drain(ipfs.get(CID.parse('QmPDqvcuA4AkhBLBuh2y49yhUB98rCnxPxa3eVNC1kAbS1'), {
          timeout: 1
        })))
      })
      ...
      it('should get with ipfs path, as object and nested value', async () => {
        ...
        const output = await pipe(
          ipfs.get(`/ipfs/${fileAdded.cid.toString()}/testfile.txt`),
          tarballed,
          collect
        )
    ...
    describe('directories', function () {
      it('should get a directory', async function () {
        ...
        const { cid } = root
        expect(cid.toString()).to.equal(fixtures.directory.cid.toString())
        const output = await pipe(
          ipfs.get(cid),
          tarballed,
          collect
      ...
      it('should get a nested directory', async function () {
        ...
        expect(cid.toString()).to.equal('QmVMXXo3c2bDPH9ayy2VKoXpykfYJHwAcU5YCJjPf7jg3g')
        const output = await pipe(
          ipfs.get(cid),
          tarballed,
          collect
