...
import type { AddProgressFn } from '../root.js'
import type { Mtime, MtimeLike } from 'ipfs-unixfs'
import type { Client } from '../lib/core.js'
import type { Version as CIDVersion } from 'multiformats/cid'

export function createFiles (client: Client): FilesAPI {
  return {
    cp: createCp(client),
...
export interface FilesAPI {
   ...
   * ```
   */
  read: (ipfsPath: IPFSPath, options?: ReadOptions) => AsyncIterable<Uint8Array>
   ...
   * ```
   */
  flush: (ipfsPath: string, options?: ClientOptions) => Promise<CID>
   ...
   * ```
   */
  ls: (ipfsPath: IPFSPath, options?: ClientOptions) => AsyncIterable<MFSEntry>
}
...
export interface ChmodOptions extends MFSOptions, ClientOptions {
  /**
...
export interface MvOptions extends MFSOptions, ClientOptions {
  /**
   * Create intermediate MFS paths if they do not exist
   */
  parents?: boolean
